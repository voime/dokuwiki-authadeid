...
//require_once(DOKU_PLUGIN.'authad/auth.php');
 ...
 */
//class auth_plugin_authadeid extends DokuWiki_Auth_Plugin {
class auth_plugin_authadeid extends auth_plugin_authad {
    ...
    public function __construct() {
        ...
        // we load authad plugin configuration
        if(!plugin_isdisabled('authad')) {
            $authad_plugin = plugin_load('auth', 'authad');
            $this->conf = $authad_plugin->conf;
        }else{
            $this->loadConfig();
        }
    ...
    public function checkPass($user, $pass) {
        //echo "authadeid checkPass<br>";

        if(isset($_SESSION['eid_username']) &&
            $pass == 'eid' &&
            $_SESSION['eid_username'] == $user) {

            $adldap = $this->_adldap($this->_userDomain($user));
            if(!$adldap) return false;

                $ret = $adldap->authenticate($this->opts["admin_username"], $this->opts["admin_password"]);
                //var_dump($ret);
                //exit;
                return $ret;
    ...
    public function getUserData($user, $requireGroups=true) {
        ...
        if(!isset($_SESSION['eid_username']) && $_SESSION['eid_username'] != $user) {
            return false;
        }
